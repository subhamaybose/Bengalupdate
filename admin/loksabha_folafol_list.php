	...
	<tr>
        ...
        <div id="content-table-inner">
			...
			<div id="table-content">
				...
				<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td class="green-left"> <?php if(isset($_REQUEST['msg'])){if($_REQUEST['msg']=='1'){ echo "Deleted Successfully";}else if($_REQUEST['msg']=='2'){ echo "Status Changed Sussessfully";}else if($_REQUEST['msg']=='3'){  echo"Added Successfully";}else if($_REQUEST['msg']=='4'){  echo"Added to Archive Successfully";}}?> <a href="loksabha_folafol_entry.php">Add new Result.</a></td>
                ...
                <?php $sql=mysql_query("SELECT loksabha_result.id, rajjo.rajjo, loksabha_constituency.constituency, loksabha_result.ca_dol_1, loksabha_result.vo_dol_1, loksabha_result.ca_dol_2, loksabha_result.vo_dol_2, loksabha_result.ca_dol_3, loksabha_result.vo_dol_3, loksabha_result.ca_dol_4, loksabha_result.vo_dol_4, loksabha_result.ca_dol_5, loksabha_result.vo_dol_5, loksabha_result.status, loksabha_result.round FROM `loksabha_result` LEFT JOIN rajjo ON loksabha_result.state=rajjo.id LEFT JOIN loksabha_constituency ON loksabha_result.kendro=loksabha_constituency.id ORDER BY loksabha_result.id DESC");
				...
				<tr class="alternate-row">					
					...
					<td><a href="loksabha_folafol_acttion.php?act=status&id=<?php echo $sqlf->id;?>&st=<?php echo $sqlf->status;?>"><?php echo ($sqlf->status=='0')?'Show':'Hide';?></a></td>

					<td>
					<a href="loksabha_folafol_entry.php?id=<?php echo $sqlf->id;?>" onclick="return confirm('Are you sure you want to edit this item?');" title="Edit" class="icon-1 info-tooltip"></a>
					<a href="loksabha_folafol_acttion.php?act=delete&id=<?php echo $sqlf->id;?>" onclick="return confirm('Are you sure you want to delete this item?');" title="Delete" class="icon-2 info-tooltip"></a>
					<a href="loksabha_folafol_acttion.php?act=archive&id=<?php echo $sqlf->id;?>" onclick="return confirm('Are you sure you want to send this item to archive?');" title="Archive" class="icon-4 info-tooltip"></a>
					</td>
				</tr>
                <?php }}?>
				</table>
