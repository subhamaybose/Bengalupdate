...
if(!isset($_POST['update'])){
			...
			$sql=mysql_query("update `municipality_result` set 
			`state`='".$_POST['state']."',
			`district`='".$_POST['district']."',
			`kendro`='".$_POST['kendro']."',
			`ward`='".$_POST['ward']."',
			...
			echo "<script>window.location.href='municipality_folafol_list.php';</script>";
...
<div id="page-heading"><h1>Add Edit Municipality Result</h1></div>
...
<tr>
    ...
    <input type="hidden" name="id" id="id" value="<?php echo  $_REQUEST['id'];?>" />
		...
		<tr>
			...
			<td colspan="2">
			<select  class="" id="district" name="district"  style="width:300px; padding:5px;" onChange="javascript:getMunicipalityE(this.value);">  
				<?php 
		...
		<tr>
			...
			<select  class="" id="kendro" name="kendro"  style="width:300px; padding:5px;">  
				<?php 
				$qls=mysql_query("select * from `municipality` where `status`='0' and `district`='".$sqllf->district."'");
				if(mysql_num_rows($qls)>0){
				while($df=mysql_fetch_object($qls)){
				?>
				<option value="<?php echo $df->id;?>" <?php if($df->id==$sqllf->kendro){?> selected<?php }?>><?php echo $df->municipality;?></option>
				<?php }}?>
			</select>
			</td>
		</tr>
		<tr>
			<th valign="top">ওয়ার্ড নম্বর :</th>
			<td colspan="2">
			<select  class="" id="ward" name="ward"  style="width:300px; padding:5px;">  
				<option value="0" selected="" disabled="">Select Ward</option>
				<?php
				for($i=1;$i<=150;$i++){
					?>
					<option value="<?php echo $i;?>" <?php if( $i==$sqllf->ward){?> selected<?php }?>>ওয়ার্ড নম্বর <?php echo $i;?></option>
					<?php
				}
				?>
			</select>
	...
	<tr>
		...
		<td valign="top">
			...
			<input type="button" value="" class="form-back"  onClick="window.location.href='municipality_folafol_list.php'" />
		</td>
		<td></td>
	</tr>
             </table>
    ...
    <table border="0" cellpadding="0" cellspacing="0"  id="id-form">  
		...
		<tr>
			<th valign="top">জেলার  নাম :</th>
			<td colspan="2">
			<select  class="" id="district" name="district"  style="width:300px; padding:5px;" onChange="javascript:getMunicipality(this.value);">  
		...
		<tr>
			...
			<select  class="" id="ward" name="ward"  style="width:300px; padding:5px;">  
				...
				for($i=1;$i<=150;$i++){
					?>
					<option value="<?php echo $i;?>">ওয়ার্ড নম্বর <?php echo $i;?></option>
					<?php
				}
				?>
			</select>
...
function getMunicipalityE(value){
	var district =value;
	var id = document.getElementById("id").value;
	$.ajax({
			url:"ajax/mkendro.php",
