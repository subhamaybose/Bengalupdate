...
if(!isset($_POST['update'])){
			echo $sql=mysql_query("insert into `tickertext` set 
			...
			$sql=mysql_query("update `tickertext` set 
			`text`='".addslashes($_POST['text'])."',
			`cat`='".$_POST['cat']."',
			`date`='".date("Y-m-d",time())."',
			`order`='".$_POST['order']."'
			...
			echo "<script>window.location.href='tickerlist.php';</script>";
...
<tr>
	...
	<td>
        ...
        <?php if(isset($_REQUEST['id'])&&$_REQUEST['id']!=''){
			$sqll= mysql_query("select * from `tickertext` where `id`='".$_REQUEST['id']."' ");
	...
	<tr>
		...
		<td valign="top">
			...
			<input type="button" value="" class="form-back"  onClick="window.location.href='tickerlist.php'" />
		</td>
		<td></td>
	</tr>
	</table>
