...
<?php 
function addHit(){
	mysql_query("INSERT INTO angana_hit_count ( pge_name, views) VALUES ( 'id=".$_GET['id']."', 3500) ON DUPLICATE KEY UPDATE views=views+1");
}
addHit();
function getHit(){
	$hits = mysql_query("SELECT * FROM `angana_hit_count` WHERE `pge_name`='id=".$_GET['id']."'");
...
<body>
	...
	<div class="row">
		...
		<div class="span8">
			<?php 
				$sql=mysql_query( "select * from `angana` where `id`='".$_REQUEST['id']."'"); 
