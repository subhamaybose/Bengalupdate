	...
	<div class="span8">
		<div class="insidecont">
			...
			<ul class="sectionBlist2">
				...
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='8' and `date`<='".$_REQUEST['dt']."' order by `id` desc limit ".$lim." ,10 ");
				...
				<li>
					...
					<div class="cont">
						...
						<div class="clear"></div> <a href="post_details-page.php?id=<?php echo $ff->id;?>&dt=<?php $_REQUEST['dt']?>">বিস্তারিত পড়ুন</a> </div>
					<div class="clear"></div>
				</li>
				<?php }}
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='8' and `date`<='".$_REQUEST['dt']."' order by `id` desc ");
				$num=mysql_num_rows($sql);
				$baseurl=$siteurl.'education.php?id='.(!empty($_GET['id'])).'&dt='.$_REQUEST['dt'];
