	...
	<div class="span8">
		<div class="insidecont">
			...
			<ul class="sectionBlist2">
				...
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='38' and `date`<='".$_REQUEST['dt']."' order by `id` desc limit ".$lim." ,10 ");
				...
				<?php }}
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='38' and `date`<='".$_REQUEST['dt']."'  order by `id` desc ");
				$num=mysql_num_rows($sql);
				$baseurl=$siteurl.'features.php?id='.(!empty($_GET['id'])).'&dt='.$_REQUEST['dt'];
