<div class="conteiner">
	<!--english news weblink start-->
	<!--<a target="_blank" href="http://bengalupdatenews.com/" class="banrightB">
		<div class="thumb"><img src="../images/logo_jpg.jpg" /> </div>
		<div class="clear"></div>
		<h3>রাজ্য রাজনীতি বেঙ্গল আপডেট ইংরেজি সংস্করন</h3> 
		<span>www.bengalupdatenews.com</span>
	</a>-->
	<!--english news weblink end-->
	<div class="clear"></div>
	<!--Google Search bar start-->
	<div class="heading-panelTwo assbg"> <span class="" style="margin-left:-15px;"><img src = "../images/google.png"></span> </div>
	<div class="clear"></div>
	<div class="section">
		<div class="" style="">
	...
	<div class="section">
		<?php 
			$sql=mysql_query( "select * from `sompadokio` where status='0' and date<='".$_REQUEST['dt']."' order by id desc limit 1"); 
			if(mysql_num_rows($sql)>0){ 
			$ff=mysql_fetch_object($sql);
		?>
		<a href="sampadaker_kalam.php?id=<?php echo $ff->id;?>&dt=<?php echo $_REQUEST['dt']?>" class="sampkalm">
			<div class="thumb">
		...
		<?php }else{?>
		<a href="#" class="sampkalm">	
			<div class="thumb">
				<img src="../images/no_img.png" alt=""/>
			</div>
			<h2><?php echo "No Post."?></h2>
		</a>	
		<?php }?>
		<div class="clear"></div>
	</div>
	...
	<div class="section">
		<div class="fotogal">
			<div id="banner-fade">
				<ul class="bjqs">
					<li>
						<a href="photo_gallery.php?dt=<?php echo $_REQUEST['dt']?>"><img src="../images/demo1.jpg" title=""> </a>
					</li>
					<?php 
					...
					?>
					<li>
						<a href="photo_gallery.php?dt=<?php echo $_REQUEST['dt']?>"> <img src="../images/uploaded/gal/<?php echo $ffs->img;?>" title=""> </a>
					</li>
					<?php }?>
				</ul>
			</div>
	...
	<div class="section">
		<?php 
			$sql=mysql_query( "select * from `angana` where status='0' and date<='".$_REQUEST['dt']."' order by id desc limit 1"); 
			if(mysql_num_rows($sql)>0){ 
			$ff=mysql_fetch_object($sql);
		?>
		<a href="angana.php?id=<?php echo $ff->id;?>&dt=<?php echo $_REQUEST['dt']?>" class="sampkalm">
			<div class="thumb">
				<?php if(!empty($ff->topicimg)){?>
				<img src="images/uploaded/<?php echo $ff->topicimg;?>" alt="<?php echo $ff->title;?>"/> 
				<?php }else{?>
	...
	<div class="section">
		<div id="q">
		<?php $sql=mysql_query("select * from `poll` where `status`='0' and date<='".$_REQUEST['dt']."' order by id desc limit 1");
		...
		<div id="a">
			<?php
			$getinfo=mysql_query("select yes,no,not_known from poll where `status`='0' and date<='".$_REQUEST['dt']."' order by id desc limit 1");
	...
	<div class="clear"></div>
	<div class="section">
		<div class="sensex"><iframe id="moneywiz_widget" name="moneywiz_widget" src="//money.rediff.com/widget/moneywizwidget" scrolling="no" frameborder="0" marginHeight="0" marginWidth="0" style="width:240px; height:310px"></iframe><!--<script type="text/javascript">var _mcq=["6",""];</script><span id='_mc_mg6'></span><script language="JavaScript" src="http://stat1.moneycontrol.com/mcjs/common/mc_widget.js"></script><noscript><a href="http://www.moneycontrol.com">Sensex/Nifty</a></noscript>--><!--<img src="../images/sensex.jpg" />--></div>
		<div class="clear"></div>
	</div>
	<!--share bazar end-->
	<div class="clear"></div>
