	...
	<div class="span8">
		<div class="insidecont">
			...
			<ul class="list-page-dtls">
				...
				$sql=mysql_query( "select * from `post` where `status`='0' and `exclusive`='1' and (`pagecat`='bangali kothai' OR `pagecat`='entertainment' OR `pagecat`='rajnity' OR `pagecat`='offbeat' OR `pagecat`='sports') and date<='".$_REQUEST['dt']."' order by `id` desc limit ".$lim." ,10 "); 
				...
				<?php } 
				$sql=mysql_query("select * from `post` where `status`='0' and `exclusive`='1' and (`pagecat`='bangali kothai' OR `pagecat`='entertainment' OR `pagecat`='rajnity' OR `pagecat`='offbeat' OR `pagecat`='sports') and date<='".$_REQUEST['dt']."' order by `id` desc ");
				$num=mysql_num_rows($sql);
				$baseurl=$siteurl.'khobore_jholok.php?id='.(!empty($_GET['id'])).'&dt='.$_REQUEST['dt'];
