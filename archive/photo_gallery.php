    	...
    	<div class="insidecont">
			...
			<li>
					...
					<div class="bb-bookblock">
						<?php
						$sqls=mysql_query("select * from `gallerys` where `status`='0' and `cat`='".$ff->cid."' and `date`<='".$_REQUEST['dt']."' order by `id` desc");
						$catid=$ff->cid;
						$catname=$ff->name;
						$cnt=0;
						while($ffs=mysql_fetch_object($sqls))
						{
							...
							if(!empty($ffs->img)){
						?>
						<div class="bb-item"><a href="cartoon-gallery.php?id=<?php echo $catid;?>&name=<?php echo $catname;?>&dt=<?php echo $_REQUEST['dt'];?>"><img src="<?php echo '../images/uploaded/gal/'.$ffs->img;?>" alt="image<?php echo $cnt;?>"/></a></div>
						<?php
							}
						}
						?>
