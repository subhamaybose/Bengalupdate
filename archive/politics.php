...
<div class="row">
	...
	<div class="breaking-news">
		...
		<div class="breaking-block">
			<div class="ticker">
				...
				<ul>
				   <?php $sql=mysql_query("select * from `scrooltext` where `status`='0' and `cat`='rajnity' and date<='".$_REQUEST['dt']."' limit 5");
    ...
    <div class="ticker-news">
        <div class="news_feed">
            <ul id="ticker01">
                <?php $sql=mysql_query("select * from `tickertext` where `status`='0' and `cat`='rajnity' and date<='".$_REQUEST['dt']."' order by `id` limit 5");
				if(mysql_num_rows($sql)>0){
				while($ff=mysql_fetch_object($sql)){
				?>
				<li><?php echo $ff->text;?></li>
	...
	<div class="span8">
		...
		<div class="camera_wrap camera_azure_skin" id="camera_wrap_2">
			<?php $sql=mysql_query("select * from `banner` where `status`='0' and `cat`='rajnity' and date<='".$_REQUEST['dt']."' order by `id` limit 6");
					$i=0;
					if(mysql_num_rows($sql)>0){
					while($ff=mysql_fetch_object($sql)){
					$i++;
		...
		<div class="insidecont">
			<div class="heading-panelTwo blueCbg m-bottom">রাজনীতি</div>
			<div class="clear"></div>
			<!--lead news start-->
			...
			<ul class="sectionBlist2">
				<?php
				$sql=mysql_query("select * from `post` where `status`='0' and `lead`='1' and `pagecat`='rajnity'and date<='".$_REQUEST['dt']."' order by `id` desc limit 1");
				if(mysql_num_rows($sql)>0){
				while($ff=mysql_fetch_object($sql)){
				?>
				<li>
			...
			<ul class="sectionBlist2">
				<?php
				$sql=mysql_query("select * from `post` where `status`='0' and `dinersera`='1' and `pagecat`='rajnity'and date<='".$_REQUEST['dt']."' order by `id` desc limit 1");
				if(mysql_num_rows($sql)>0){
				while($ff=mysql_fetch_object($sql)){
				?>
				<li>
			...
			<div class="heading-panel">
				...
				<div class="panelR"> <a href="politics_state.php?dt=<?php echo $_REQUEST['dt'];?>">আরও খবর দেখুন</a> </div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<ul class="sectionBlist2">
				<?php
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='30'and date<='".$_REQUEST['dt']."' order by `id` desc limit 2");
				if(mysql_num_rows($sql)>0){
				while($ff=mysql_fetch_object($sql)){
				?>
				<li>
			...
			<div class="clear"></div>
			<div class="heading-panel">
				<div class="panelL redbg">দেশ</div>
				<div class="panelR"> <a href="politics_country.php?dt=<?php echo $_REQUEST['dt'];?>">আরও খবর দেখুন</a> </div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<ul class="sectionBlist2">
				<?php
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='31'and date<='".$_REQUEST['dt']."' order by `id` desc limit 2");
				if(mysql_num_rows($sql)>0){
				while($ff=mysql_fetch_object($sql)){
				?>
				<li>
			...
			<div class="clear"></div>
			<div class="heading-panel">
				<div class="panelL greenBbg">আন্তর্জাতিক</div>
				<div class="panelR"> <a href="politics_international.php?dt=<?php echo $_REQUEST['dt'];?>">আরও খবর দেখুন</a> </div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
			<ul class="sectionBlist2">
				<?php
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='32'and date<='".$_REQUEST['dt']."' order by `id` desc limit 2");
				if(mysql_num_rows($sql)>0){
				while($ff=mysql_fetch_object($sql)){
				?>
				<li>
			...
			<div class="">
				<ul class="sectioninlist">
					<?php 
					$sql=mysql_query("select * from `post` where `status`='0' and `exclusive`='1' and `pagecat`='rajnity' and date<='".$_REQUEST['dt']."' order by `id` desc limit 2");
					if(mysql_num_rows($sql)>0){
					while($ff=mysql_fetch_object($sql)){
					?>
					<li>
			...
			<ul class="sectionBlist2">
				<?php
				$sql=mysql_query("select * from `post` where `status`='0' and `topnews`='1' and `pagecat`='rajnity'and date<='".$_REQUEST['dt']."' order by `id` desc limit 2");
				if(mysql_num_rows($sql)>0){
				while($ff=mysql_fetch_object($sql)){
				?>
				<li>
