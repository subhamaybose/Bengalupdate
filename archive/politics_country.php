	...
	<div class="span8">
		<div class="insidecont">
			...
			<ul class="sectionBlist2">
				...
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='31'and date<='".$_REQUEST['dt']."' order by `id` desc limit ".$lim." ,10 ");
				...
				<li>
					<div class="thumb">
						...
						<?php } else{?>
						<img src="../images/no_img.jpg" alt="<?php echo $ff->title;?>" />
						<?php } ?>
					</div>
					<div class="cont">
				...
				<?php }}
				$sql=mysql_query("select * from `post` where `status`='0' and `cat`='31'and date<='".$_REQUEST['dt']."'  order by `id` desc ");
				$num=mysql_num_rows($sql);
				$baseurl=$siteurl.'politics_country.php?id='.(!empty($_GET['id'])).'&dt='.$_REQUEST['dt'];
