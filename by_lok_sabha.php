				...
				<table width="100%" class="Vtable">
					<thead>
						...
						<tr>
							...
							?>
								<h1>উপনির্বাচন -লোকসভার ফলাফল (<span><?php echo $sqlf1->loksabha;?></span> টি আসন) <span><?php echo $_REQUEST['yr'];?></span></h1>
							<?php }}?>
					...
					<tbody>
					<?php $sql=mysql_query("SELECT by_election_loksabha_result.id, rajjo.rajjo, loksabha_constituency.constituency, by_election_loksabha_result.ca_dol_1, by_election_loksabha_result.vo_dol_1, by_election_loksabha_result.ca_dol_2, by_election_loksabha_result.vo_dol_2, by_election_loksabha_result.ca_dol_3, by_election_loksabha_result.vo_dol_3, by_election_loksabha_result.ca_dol_4, by_election_loksabha_result.vo_dol_4, by_election_loksabha_result.ca_dol_5, by_election_loksabha_result.vo_dol_5, by_election_loksabha_result.status, by_election_loksabha_result.round, by_election_loksabha_result.result, by_election_loksabha_result.difference FROM `by_election_loksabha_result` LEFT JOIN rajjo ON by_election_loksabha_result.state=rajjo.id LEFT JOIN loksabha_constituency ON by_election_loksabha_result.kendro=loksabha_constituency.id WHERE by_election_loksabha_result.state='".$_REQUEST['state']."' AND by_election_loksabha_result.date='".$_REQUEST['yr']."' AND by_election_loksabha_result.status='0' ");
					if(mysql_num_rows($sql)==0){
					?>
					<tr>					
